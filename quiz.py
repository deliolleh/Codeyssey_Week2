...
def normalize(text):
    ...
    NFC 정규화도 함께 한다. 한글은 저장 방식이 두 가지다.
        NFC  '통영' = 완성된 글자 2개
        NFD  '통영' = 자모 6개 (ㅌ ㅗ ㅇ ㅇ ㅕ ㅇ)
    화면에는 똑같이 보이지만 파이썬은 다른 문자열로 본다.
    macOS가 파일명 등에 NFD를 쓰기 때문에, 다른 앱에서 복사해 붙여넣으면
    정답인데 오답 처리되는 일이 생긴다. 비교 전에 한쪽으로 모아준다.
